...
//! projects DB, Ollama client) and the storage paths derived from the
//! OS app-data dir.
...
pub struct AppPaths {
    ...
    pub project_storage_dir: PathBuf,
}
...
impl AppPaths {
    pub fn from_handle(app: &AppHandle) -> Result<Self> {
        ...
        Ok(Self {
            ...
            projects_db: data_dir.join("projects.db"),
            ...
            project_storage_dir: data_dir.join("storage"),
            data_dir,
        })
    }
}
...
impl AppState {
    pub fn build(app: &AppHandle) -> Result<Self> {
        ...
        std::fs::create_dir_all(&paths.project_storage_dir).ok();
        let audit = AuditDb::open(&paths.audit_db)?;
        let projects = ProjectsDb::open(&paths.projects_db)?;
        Ok(Self {
            audit,
            projects,
